...
\lstdefinelanguage{MASM}{
    morekeywords={PROC,ENDP,END,PUSH,POP,MOV,ADD,SUB,CALL,RET,JMP,
                  ...
                  EQU,.586,.model,.data,.code,flat,stdcall,offset,option,
                  casemap,none,include,includelib,start},
    sensitive=false,
    morecomment=[l]{;}
}
...
\begin{figure}[H]
    ...
    \resizebox{0.8\textwidth}{!}{%
        \begin{tikzpicture}[
            ...
            \node[process,   right=2.2cm of loop]    (suma)   {EAX $\leftarrow$ EAX $+$ [ESI]};
            \node[process,   below=0.8cm of suma]    (avanza) {ESI $\leftarrow$ ESI $+$ 4,\ ECX$--$};
            \node[process,   below=0.8cm of loop]    (cdq)    {CDQ: extiende signo EAX $\to$ EDX:EAX};
            ...
            \draw[arr] (loop)    -- node[above, font=\scriptsize]{Sí} (suma);
            \draw[arr] (suma)    -- (avanza);
            \draw[arr] (avanza.west) -- ++(-0.5,0) |- (loop.east);
            \draw[arr] (loop)    -- node[right, font=\scriptsize]{No} (cdq);
            \draw[arr] (cdq)     -- (div);
            \draw[arr] (div)     -- (guarda);
            \draw[arr] (guarda)  -- (fin);
        \end{tikzpicture}%
...
sumar_loop:
    ...
    loop sumar_loop     ; Decrementa ECX y salta si ECX > 0
...
\begin{table}[H]
    ...
    \begin{tabular}{@{}cccc@{}}
        ...
        \bottomrule
    \end{tabular}
    \caption{Evolución del acumulador durante el ciclo \texttt{sumar\_loop}}
\end{table}
...
https://github.com/7mo-ArquitecturaComputadoras/Practica02\_Promedio
...
\begin{itemize}
    ...
    \item La instrucción \texttt{CDQ} es indispensable antes de \texttt{IDIV}: sin ella, \texttt{EDX} contendría valores arbitrarios y el resultado de la división sería incorrecto, especialmente con sumas negativas.
